import {
  ...
  useToast, VStack, Badge, FormErrorMessage,
...
// ... (interfaces e lógica do componente permanecem as mesmas)
...
export const FormularioNovaVenda = ({ isOpen, onClose, vendaParaEditar }: FormularioNovaVendaProps) => {
  ...
  return (
    <Drawer isOpen={isOpen} placement="right" onClose={onClose} size={drawerSize}>
      <DrawerOverlay />
      {/* ✅ CORREÇÃO 1: Aplicando Flexbox ao DrawerContent */}
      <DrawerContent as="form" onSubmit={handleSubmit(onSubmit)} display="flex" flexDirection="column" h={{ base: "100vh", md: "auto" }}>
        <DrawerHeader borderBottomWidth="1px">{vendaParaEditar ? 'Editar Venda' : 'Registrar Nova Venda'}</DrawerHeader>
        <DrawerCloseButton />
        
        {/* ✅ CORREÇÃO 2: Aplicando flex="1" e overflowY="auto" ao DrawerBody */}
        <DrawerBody flex="1" overflowY="auto">
          <VStack spacing={4} align="stretch">
            {/* Todo o conteúdo do formulário permanece aqui dentro */}
              ...
              <FormControl isRequired isInvalid={!!errors.cliente_id} flex={1}>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (getValues('cliente_id')) setValue('cliente_id', '');
                  }}
                />
                {isLoadingClientes && <Spinner size="sm" mt={2} />}
                {clientes && termoBuscaCliente && getValues('cliente_id') === '' && (
                  <VStack mt={1} borderWidth={1} borderRadius="md" maxH="150px" overflowY="auto" align="stretch" spacing={0} bg={useColorModeValue('white', 'gray.700')} zIndex="dropdown" position="absolute" width="98%">
                    {clientes.dados.length > 0 ? clientes.dados.map((c: ICliente) => (
                      <Button key={c.id} onClick={() => { setValue('cliente_id', String(c.id), { shouldValidate: true }); setTermoBuscaCliente(c.nome); }} justifyContent="flex-start" variant="ghost" borderRadius={0}>
                        {c.nome}
                      </Button>
            ...
            <Box p={4} borderWidth={1} borderRadius="md" mt={4}>
              ...
              <Flex direction={flexDir} gap={2} align="flex-end">
                ...
                <FormControl flex={1}>
                  ...
                  <Input isReadOnly value={estoqueAtual !== null ? estoqueAtual : '---'} textAlign="center" fontWeight="bold" bg={useColorModeValue('gray.100', 'gray.600')} borderColor={estoqueAtual !== null && estoqueAtual <= 0 ? 'red.500' : useColorModeValue('gray.200', 'gray.600')} color={estoqueAtual !== null && estoqueAtual <= 0 ? 'red.500' : 'inherit'} />
                </FormControl>
                <FormControl flex={1}>
                  <FormLabel>Qtd/ Peso</FormLabel>
                  <Controller name="quantidade" control={control} render={({ field }) => (<NumberInput {...field} min={0.001} precision={3}><NumberInputField placeholder="Info Qtd/Peso" /></NumberInput>)} />
                </FormControl>
                <FormControl flex={1}>
              ...
              {produtosNaVenda.map(p => (
                <Flex key={p.produto_id} justify="space-between" align="center" p={2} borderWidth={1} borderRadius="md">
                  <Box>
                    ...
                    <Text fontSize="sm" color="gray.500">{p.quantidade} {p.unidade_medida} x {(p.preco_manual ?? p.preco_original).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} {p.preco_manual !== undefined && <Badge ml={2} colorScheme="orange">Manual</Badge>}</Text>
                  </Box>
            ...
            <Flex justify="flex-end" mt={4}>
              <Box textAlign="right">
                ...
                <Heading size="lg" color="teal.500">Total: {valorTotalComItemAtual.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</Heading>
              </Box>
            </Flex>
          </VStack>
        </DrawerBody>
